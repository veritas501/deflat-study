...
class DeFlat:
    ...
    #         self.nodes_info[node] = tmp_super_node
    #     # 获取各种关键node
    #     self.prologue_node = filter(
    #         lambda n: self.nodes_info[n].in_degree == 0,
    #         self.nodes_info
    #     ).__next__()
    #     print(f'[+] prologue_node: {hex(self.prologue_node.addr)}')
    #     self.ret_node = filter(
    #         lambda n: self.nodes_info[n].out_degree == 0,
    #         self.nodes_info
    #     ).__next__()
    #     print(f'[+] ret_node: {hex(self.ret_node.addr)}')
    #     self.main_dispatcher_node = self.nodes_info[self.prologue_node].successors[0]
    #     print(f'[+] main_dispatcher_node: {hex(self.main_dispatcher_node.addr)}')
    #     self.pre_dispatcher_node = filter(
    #         lambda n: n != self.prologue_node,
    #         self.nodes_info[self.main_dispatcher_node].predecessors
    #     ).__next__()
    #     print(f'[+] pre_dispatcher_node: {hex(self.pre_dispatcher_node.addr)}')
    #     self.relevant_nodes = self.nodes_info[self.pre_dispatcher_node].predecessors
    #     if self.p.arch.name in ASM.ARCH_X86:
    #         self.relevant_nodes = list(filter(
    #             lambda n: self.p.factory.block(n.addr, size=n.size).capstone.insns[0].mnemonic != 'jmp',
    #             self.relevant_nodes
    #         ))
    #     else:
    #         raise Exception("arch not support")
    #     print(f'[+] relevant_nodes: {list(map(lambda x: hex(x.addr), self.relevant_nodes))}')
    #     self.nop_nodes = list(self.cfg.nodes)
    #     self.nop_nodes.remove(self.prologue_node)
    #     self.nop_nodes.remove(self.ret_node)
    #     self.nop_nodes.remove(self.main_dispatcher_node)
    #     self.nop_nodes.remove(self.pre_dispatcher_node)
    #     for node in self.relevant_nodes:
    #         self.nop_nodes.remove(node)
    #     print(f'[+] nop_nodes: {list(map(lambda x: hex(x.addr), self.nop_nodes))}')
    #     self.useful_nodes = [self.prologue_node, self.ret_node] + self.relevant_nodes
    #     self.useful_nodes_except_ret = [self.prologue_node] + self.relevant_nodes
